...
<head>
    ...
    <title>Temas Pendientes RRHH</title>
     @vite(['resources/css/app.css', 'resources/js/hr_search.js'])
...
<body class="min-h-screen flex flex-col bg-body">
    ...
    @auth
        ...
        <main class="grow flex w-full flex-1">
            ...
            <section class="flex flex-col items-center w-full overflow-y-auto min-h-0">
                <!-- Capçalera -->
                ...
                <div class="w-11/12 my-6 flex items-center justify-between p-4 bg-white rounded-lg shadow-sm">
                    <div class="relative w-3/4">
                        <input type="search" 
                            id="search_rrhh"
                            name="text"
                            placeholder="Cercar per professional, context o descripció..." 
                    ...
                    <select id="status_filter" class="bg-white border border-gray-300 rounded-lg px-4 py-3 shadow-sm focus:border-[#ff7300] focus:outline-none">
                        <option value="tots">Tots els estats</option>
                        <option value="in_process">En process</option>
                ...
                <!-- Contenidor de temes (aquí s'actualitzarà amb JS) -->
                <div id="issues_container" class="w-11/12 flex flex-col space-y-6 mb-10 overflow-y-auto flex-1 min-h-0">
                    <!-- Contingut inicial (es mostra quan es carrega la pàgina) -->
                    @foreach($hr_pending_issues as $issue)
                        <div class="bg-white rounded-xl shadow-md border border-gray-300 overflow-hidden hover:shadow-lg transition-shadow duration-300">
                            ...
                            <div class="px-6 pt-4 flex justify-between border border-[#f0f0f0] bg-gray-50">
                                ...
                                @else
                                    <span class="px-3 py-1 bg-orange-100 text-orange-800 rounded-2xl text-sm font-medium flex items-center h-max">
                                        Pendent
                                    </span>
                                @endif
                            </div>
